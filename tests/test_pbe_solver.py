...
class TestPBESolver:
    ...
    def test_r_star_decreases_with_kappa(self):
        """Honeypot baiting rate decreases as kappa rises: higher attack threshold means less baiting needed."""
        results = kappa_sweep(p_real=0.4, V=10.0, C=3.0, L=5.0)
        kappas  = sorted(results.keys())
        r_stars = [results[k].r_star for k in kappas]
        assert r_stars == sorted(r_stars, reverse=True), "r* should decrease with kappa"
